...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def fastv_forward(
        ...
        if use_cache and inputs_embeds.shape[1] != 1:
            logger.warning_once("FastV/VLA-Pruner full-sequence pruning disables KV cache for correctness.")
            use_cache = False
        ...
        pruning_info = {
            ...
            "kept_indices": None,
            "pruning_layer": None,
            "mode": "vla_pruner" if bool(fastv_config.get("use_vla_pruner", False)) else "fastv",
        }
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if (
                ...
                if last_layer_attention is None:
                    raise ValueError(
                        "FastV/VLA-Pruner requires attention weights from output_attentions=True."
                    )
                ...
                if keep_indices.shape[0] < seq_length:
                    ...
                    if use_cache:
                        logger.warning_once("FastV/VLA-Pruner disables KV cache after visual token pruning.")
                        use_cache = False
                        past_key_values = None
                        next_decoder_cache = None
